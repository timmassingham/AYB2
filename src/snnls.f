...
C
      ...
      SUBROUTINE SG1 (A,B,CTERM,STERM,SIG)   
c
...
C     ------------------------------------------------------------------
      REAL A, B, CTERM, ONE, SIG, STERM, XR, YR, ZERO
...
C     ------------------------------------------------------------------
      ...
      SUBROUTINE SG2    (CTERM,STERM,X,Y)
c
C  APPLY THE ROTATION COMPUTED BY SG1 TO (X,Y).  
c
c  The original version of this code was developed by
c  Charles L. Lawson and Richard J. Hanson at Jet Propulsion Laboratory
...
C     SUBROUTINE SH12 (MODE,LPIVOT,L1,M,U,IUE,UP,C,ICE,ICV,NCV)  
C   
...
C     ------------------------------------------------------------------
      SUBROUTINE SH12 (MODE,LPIVOT,L1,M,U,IUE,UP,C,ICE,ICV,NCV)  
C     ------------------------------------------------------------------
      integer I, I2, I3, I4, ICE, ICV, INCR, IUE, J
      integer L1, LPIVOT, M, MODE, NCV
      REAL B, C(*), CL, CLINV, ONE, SM
c     REAL U(IUE,M)
      REAL U(IUE,*)
      REAL UP
      parameter(ONE = 1.0d0)
...
C     SUBROUTINE SNNLS  (A,MDA,M,N,B,X,RNORM,W,ZZ,INDEX,MODE)
C   
...
C   
C     ------------------------------------------------------------------
      SUBROUTINE SNNLS (A,MDA,M,N,B,X,RNORM,W,ZZ,INDEX,MODE) 
C     ------------------------------------------------------------------
      integer I, II, IP, ITER, ITMAX, IZ, IZ1, IZ2, IZMAX, J, JJ, JZ, L
      integer M, MDA, MODE,N, NPP1, NSETP, RTNKEY
c     integer INDEX(N)  
c     REAL A(MDA,N), B(M), W(N), X(N), ZZ(M)   
      integer INDEX(*)  
      REAL A(MDA,*), B(*), W(*), X(*), ZZ(*)   
      REAL ALPHA, ASAVE, CC, SDIFF, DUMMY, FACTOR, RNORM
      REAL SM, SS, T, TEMP, TWO, UNORM, UP, WMAX
      REAL ZERO, ZTEST
...
C   
      ASAVE=A(NPP1,J)   
      CALL SH12 (1,NPP1,NPP1+1,M,A(1,J),1,UP,DUMMY,1,1,0)    
      UNORM=ZERO
      IF (NSETP .ne. 0) then
          DO 90 L=1,NSETP   
   90       UNORM=UNORM+A(L,J)**2     
      ...
      UNORM=sqrt(UNORM) 
      IF (SDIFF(UNORM+ABS(A(NPP1,J))*FACTOR,UNORM) .gt. ZERO) then
C   
...
C   
         DO 120 L=1,M  
  120        ZZ(L)=B(L)    
         CALL SH12 (2,NPP1,NPP1+1,M,A(1,J),1,UP,ZZ,1,1,1)   
...
C   
      IF (IZ1 .le. IZ2) then
         DO 160 JZ=IZ1,IZ2 
            JJ=INDEX(JZ)  
            CALL SH12 (2,NSETP,NPP1,M,A(1,J),1,UP,A(1,JJ),1,MDA,1)
  160    continue
      endif
C   
      IF (NSETP .ne. M) then
...
C   
      IF (JJ .ne. NSETP) then
         ...
         DO 280 J=JJ,NSETP 
            II=INDEX(J)   
            INDEX(J-1)=II 
            CALL SG1 (A(J-1,II),A(J,II),CC,SS,A(J-1,II))   
            A(J,II)=ZERO  
            DO 270 L=1,N  
               IF (L.NE.II) then
c
c                 Apply procedure SG2 (CC,SS,A(J-1,L),A(J,L))  
c
                  TEMP = A(J-1,L)
  ...
  270       CONTINUE  
c
c                 Apply procedure SG2 (CC,SS,B(J-1),B(J))   
c
            TEMP = B(J-1)
